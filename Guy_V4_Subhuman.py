...
def log_data():
    ...
    if name == "" or hired == "" or amount == "":
        ...
        print(f"Name: {name}, Hired: {hired}, Amount: {amount}, recepit: {recepit}, Return: {returns} ")
...
submit_btn = ttk.Button(root, text= "Save to program", command= log_data) 
submit_btn.grid(row=5, column=0, pady=10)

finish_btn = ttk.Button(root, text="Save to text file", command= save_data)
...
ttk.Label(root, text= "receipt: "). grid(row=7, column=0, sticky="e")
